...
const translations = {
    ...
    de: {
        ...
        accountCreated:
            "Demo-Account wurde erstellt. Du kannst dich jetzt anmelden.",
...
function calculateConnection() {
    ...
    const timeText =
        hours > 0
            ? `${hours} ${t("hours")} ${minutes > 0 ? `${minutes} ${t("minutes")}` : ""}`
            : `${minutes} ${t("minutes")}`;
...
function calculateTimezone() {
    ...
    try {
        ...
        const formatted =
            `${values.day}.${values.month}.${values.year} ${values.hour}:${values.minute}`;
...
function updateAirlineFinderLanguage() {
    ...
    /*
     * All static Airline Finder text uses
     * data-i18n and is therefore updated by
     * applyLanguage().
     */
